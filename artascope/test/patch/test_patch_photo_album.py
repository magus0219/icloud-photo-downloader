...
class MockResponse:
    ...
    def json(self):
        if self.idx < 0 or self.idx > len(MOCK_PHOTO_ASSET_DATA) + 1:
            return {"records": []}
...
class TestPatchPhotoAlbum:
    def test_get_photos_by_date_desc(self, mock_album):
        ...
        assert len(photos) == len(mock_data) // 2
        assert (
            photos[0].id
    ...
    def test_calculate_offset_and_cnt(self, mock_album, monkeypatch):
        ...
        # only date_start
        data = {
            "album_len": len(MOCK_PHOTO_ASSET_DATA) + 1,
    ...
    def test_fetch_photos(self, mock_album):
        photos = [photo for photo in mock_album.fetch_photos(offset=2, cnt=2)]
        assert len(photos) == 2
        assert (
            photos[0].id
            == MOCK_PHOTO_ASSET_DATA[1]["fields"]["masterRef"]["value"]["recordName"]
