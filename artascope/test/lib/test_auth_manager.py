...
class MockMessageException(Exception):
    pass


class MockCeleryTask:
    def delay(self, msg):
        raise MockMessageException(msg)
...
class TestAuthManager:
    ...
    def test_send_captcha_with_invalid_login_status(self, am):
        am.set_login_status(LoginStatus.CAPTCHA_RECEIVED)
        with pytest.raises(InvalidLoginStatus, match="need VERIFY_CODE_NEED"):
            am.send_captcha()
    ...
    def test_send_captcha_notify_slack(self, am, monkeypatch):
        ...
        with pytest.raises(
            MockMessageException,
            match="Goto {url_prefix}/user/captcha/<username> to enter your icloud HSA captcha!".format(
