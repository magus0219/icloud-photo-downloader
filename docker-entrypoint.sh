...
if [[ "$1" = 'celery_worker_main' ]]; then
	...
	celery -A artascope.src.celery_app worker -Q default -n worker_main@%%h -E --loglevel=info --pidfile=/var/run/%n.pid --max-tasks-per-child=100
fi

if [[ "$1" = 'celery_worker_msg' ]]; then
	# start celery worker_msg
	echo "start celery worker_msg"
	celery -A artascope.src.celery_app worker -c1 -Q msg -n worker_msg@%%h -E --loglevel=info --pidfile=/var/run/%n.pid --max-tasks-per-child=100
fi

if [[ "$1" = 'celery_worker_upload' ]]; then
	# start celery worker_upload
	echo "start celery worker_upload"
	celery -A artascope.src.celery_app worker -c1 -Q upload -n worker_upload@%%h -E --loglevel=info --pidfile=/var/run/%n.pid --max-tasks-per-child=100
fi
